...
from .arithmetic_sampling import arithmetic_categorical, make_default_codes
...
class GenerationMixin:
    ...
    The class exposes [`~generation.GenerationMixin.generate`], which can be used for:
        ...
        - *arithmetic_sampling* if `arithmetic_sampling=True`
    ...
    def _arithmetic_sample(
        ...
        **model_kwargs,
        ...
        r"""
        Generates sequences of token ids for models with a language modeling head using **arithmetic sampling** and
        ...
        if not isinstance(logits_warper, LogitsProcessorList):
            raise ValueError(
                "`logits_warper` must be a `LogitsProcessorList` instance (it is "
                f"{logits_warper})."
            )
        ...
        initial_codes = make_default_codes(input_ids.shape[0]//num_return_sequences, num_return_sequences).to(input_ids.device)
        flattened_codes = torch.flatten(initial_codes, start_dim=0, end_dim=1)
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # sample
            next_tokens, flattened_codes = arithmetic_categorical(next_token_scores, flattened_codes)
